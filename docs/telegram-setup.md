...
2. 按提示输入 Bot 名称（如 `James Assistant`）
3. 输入 Bot 用户名（必须以 `bot` 结尾，如 `james-agent_bot`）
...
```bash
export JAMES_TELEGRAM_TOKEN="1234567890:ABCdefGHIjklMNOpqrsTUVwxyz"
```
...
日志中看到以下内容表示成功：
```
[telegram] authorized as @james-agent_bot
...
- 确认 API Key 已配置（`james-agent status`）
- 如果在国内，确认代理配置正确
