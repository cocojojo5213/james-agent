...
2. **请求地址**填写你的公网 URL：
   ...
   ```
3. 飞书会自动发送 challenge 验证请求，james-agent 会自动响应
...
```bash
export JAMES_FEISHU_APP_ID="cli_a5xxxxx"
export JAMES_FEISHU_APP_SECRET="your-app-secret"
```
...
# 创建隧道
cloudflared tunnel create james-agent

# 配置 DNS
cloudflared tunnel route dns james-agent feishu-bot.yourdomain.com

# 运行
cloudflared tunnel run james-agent
```
...
**Q: 如何获取用户的 open_id？**
- 查看 james-agent 日志中的 `inbound from feishu/ou_xxx` 信息
