...
> 说明：james-agent 只实现渠道协议与业务逻辑；公网入口、证书、域名和反向代理由部署方自行配置。
...
> 可选字段：`ReceiveID`（若你明确知道加解密校验用的接收方 ID，可在 james-agent 中配置；不配则不做严格 ReceiveID 校验）
...
保存时平台会发起 URL 验证请求，james-agent 会自动处理。
...
```bash
export JAMES_WECOM_TOKEN="your-token"
export JAMES_WECOM_ENCODING_AES_KEY="your-43-char-encoding-aes-key"
export JAMES_WECOM_RECEIVE_ID="optional-receive-id"
```
...
- 出站依赖临时 `response_url`：
  - 只有在该会话最近有入站消息且缓存了 `response_url`，james-agent 才能回消息
