...
func TestLoadSkills_EmptyKeywords(t *testing.T) {
	...
	// With auto-extraction from description "no keywords", "keywords" (len 8) will be extracted
	if len(registration.Definition.Matchers) != 1 {
		t.Fatalf("matchers count = %d, want 1 (auto-extracted from description)", len(registration.Definition.Matchers))
	}
}
