	...
	for {
		...
		case evt, ok := <-qrChan:
			...
			case whatsmeow.QRChannelEventCode:
				slog.Info("whatsapp: scan the QR code below to login")
				qrterminal.GenerateHalfBlock(evt.Code, qrterminal.L, os.Stdout)
			default:
				if evt.Error != nil {
					slog.Error("whatsapp login event", "event", evt.Event, "error", evt.Error)
				} else {
...
func (w *WhatsAppChannel) handleMessage(evt *events.Message) {
	...
	if !w.IsAllowed(sender) && !w.IsAllowed(rawSender) {
		slog.Warn("whatsapp rejected message", "sender", sender)
		return
	}
...
func (w *WhatsAppChannel) extractContent(evt *events.Message) (string, []model.ContentBlock) {
	...
	if image := msg.GetImageMessage(); image != nil {
		...
		cancel()
		if err != nil {
			slog.Error("whatsapp download image failed", "error", err)
		} else if len(data) > 0 {
