...
func (t *TelegramChannel) initBot() error {
	...
	t.bot = bot
	slog.Info("telegram authorized", "username", bot.GetSelf().UserName)
	return nil
}
...
func (t *TelegramChannel) handleMessage(msg *tgbotapi.Message) {
	...
	if !t.IsAllowed(senderID) {
		slog.Warn("telegram rejected message", "sender", senderID, "username", msg.From.UserName)
		return
	}
	...
	if len(msg.Photo) > 0 {
		...
		if err != nil {
			slog.Error("telegram download photo failed", "file_id", photo.FileID, "error", err)
		} else {
	...
	if msg.Document != nil {
		...
		if err != nil {
			slog.Error("telegram download document failed", "file_id", msg.Document.FileID, "error", err)
		} else {
...
func (t *TelegramChannel) Send(msg bus.OutboundMessage) error {
	...
	for len(content) > 0 {
		...
		if _, err := t.bot.Send(tgMsg); err != nil {
			// Retry current chunk without HTML parse mode
			tgMsg.ParseMode = ""
			tgMsg.Text = chunk
