...
func TestWeComCallback_ReceiveTextMessage_OK(t *testing.T) {
	...
	nonce := "nonce-2"
	plaintext := `{"msgid":"10001","aibotid":"AIBOTID","chattype":"single","from":{"userid":"zhangsan"},"response_url":"https://example.com/resp","msgtype":"text","text":{"content":"你好，james"}}`
	...
	case msg := <-b.Inbound:
		...
		if msg.Content != "你好，james" {
			t.Errorf("content = %q, want 你好，james", msg.Content)
		}
